...
$(document).on("keyup", ".item-name-invoice", function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      data = JSON.parse(data);
      let table =
        "<ul class='item-selected list-group position-absolute bg-white'>";
      data.data.forEach(function (value) {
...
function invoiceData(page) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var table ="<div class='holding-table'><table class='table table-bordered'>";
      table +="<tr class='bg-blue'>";   
      table +="<td> Sr.No</td>";   
      table +="<td> Action</td>";   
      table +="<td> Invoice Id</td>";   
      table +="<td> Invoice No</td>";   
      table +="<td> Date</td>";   
...
function allOnes(num) {
    ...
    return str === '1'.repeat(str.length);
}
function addInvoic() {
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      data.data.forEach(function(value,index){
        ...
        input +="<tr>"
        input +=`<td><input type="text" class="item-name-invoice form-control"  onchange="fetchItemData(this)" onkeyup="changeAmt()" name='itemName[]'  value="${value['item_name']}"><div class="itemselect"></div>
         <input type="text" disabled hidden name='itm_id[]' class='itm_Id' value="${value['item_id']}">  </td>`
        input +=`<td><input type="text" class="item-price-invoice form-control"  name="price[]"  value="${value['price']}"></td>`
        input +=`<td><input type="text" class="item-quantity-invoice form-control"  onkeyup="changeAmt()" name='quantity[]'  value="${value['Quantity']}"></td>`
        let amount=Number(value['Quantity'])*Number(value['price']);
        input +=`<td><input type="text" class="rowTotal  form-control"  name="rowTotal[]" value="${amount}"></td>`
        input +='<td><button type="button" class="removeForm btn btn-outline-danger border border-0">X</button></td>'
        input +="</tr>"
        
      })
      input +="</table>"
      $('.loadmoreForm').html(input)
      let buttons="<input type='text' class='total-amount-invoice'>"
      
      buttons +="<button type='button' onclick='updateInvoice()'>"
      buttons +="Update"
      buttons +="</button>"
